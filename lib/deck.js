...
export async function buildDeck(col, opts = {}) {
  ...
  for (const row of rows) {
    ...
    const card = { cardId: id, front: q, back: a };
  ...
  if (lost.size) {
    ...
    for (const [name, list] of byDeck) {
      const kept = [];
      for (const c of list) {
        c.front = c.front.replace(dead, '');
        c.back = c.back.replace(dead, '');
        if (isEmpty(c.front) || isEmpty(c.back)) {
          drop('the picture was the whole card, and it could not be read',
            sample(c.front) || sample(c.back));
          continue;
        }
        kept.push(c);
      }
  ...
  names.forEach((name, i) => {
    ...
    const sectionId = slug(label || name, taken);
    const list = byDeck.get(name);
    sections.push({
      sectionId,
      title: cap(label || name),
      cardCount: list.length,
      order: i + 1,
    });
    for (const c of list) cards.push({ ...c, sectionId });
    if (rest.length > 1) groupOf.set(sectionId, rest[0]);
  });
  ...
  for (const [sectionId, groupName] of groupOf) {
    if (!byGroup.has(groupName)) byGroup.set(groupName, []);
    byGroup.get(groupName).push(sectionId);
  }
  if (byGroup.size > 1 && groupOf.size === sections.length) {
    for (const [name, sectionIds] of byGroup) {
      groups.push({
        groupId: slug(name, gTaken),
        title: cap(name),
        sectionIds,
        cardCount: sectionIds.reduce((sum, sectionId) =>
          sum + sections.find((section) => section.sectionId === sectionId).cardCount, 0),
      });
    }
  }
  ...
  sections.forEach((section, i) => {
    sectionArt[section.sectionId] = RAVENS[i % RAVENS.length];
  });
  const groupArt = {};
  groups.forEach((group, i) => {
    groupArt[group.groupId] = RAVENS[(i + 3) % RAVENS.length];
  });


  const deck = {
    /* Anki templates have already been rendered and sanitized above. This is
     * descriptive course data, but its text is runtime HTML rather than
     * creator-authored CommonMark. The importer projects it to permanent
     * format 1 at the compatibility boundary before storage. */
    contentRepresentation: 'sanitized-html',
    title,
    sections,
    groups,
    cards,
    buildFingerprint: hash(cards.map((card) => card.cardId).join(',') + title),
  };
